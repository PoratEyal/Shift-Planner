...
userRouter.post(`/getAllWorkers`, (req, res) => {
    ...
    User.find(
        { _id: { $nin: convertedArr }, manager: shiftWorkers.manager },
        { _id: 1, fullName: 1 },
    )
        .then((data) => {
...
userRouter.post("/getMyWorkers", (req, res) => {
    ...
    User.find({ manager: new ObjectId(job) })
        .then((data) => {
            res.status(200).json(data);
        })
        .catch((err) => {
...
userRouter.get("/GetUserRole", authenticateToken, (req, res) => {
    if (
        req.user.job === "649c08040834b0d306adef45" ||
        req.user.job === "64c259551a5f2d4dca3424bb"
    ) {
        const resRole = {
            job: "admin",
        };
        ...
        req.user.job === "649d571b70f2c12b782d204f" ||
        req.user.job === "64c2594b1a5f2d4dca3424b9"
    ) {
        const resRole = {
            job: "user",
        };
