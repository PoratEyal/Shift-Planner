...
weekRouter.post("/getCurrentWeek", async (req, res) => {
    try {
        ...
        const week = await Week.findOne({ name: "CurrentWeek", ofManager: managerId }).then(
            (response) => {
...
weekRouter.put("/setNextWeekVisible", (req, res) => {
    const managerId = req.body.id;
    Week.findOneAndUpdate({ name: "NextWeek", ofManager: managerId }, { visible: "true" })
        .then((response) => {
            res.status(200).json(response);
        })
        .catch((err) => {
...
weekRouter.put("/setNextWeekAiTrue", (req, res) => {
    const managerId = req.body.id;
    Week.findOneAndUpdate({ name: "NextWeek", ofManager: managerId }, { usedAi: "true" })
        .then((response) => {
            res.status(200).json(response);
        })
        .catch((err) => {
...
weekRouter.put("/setNextWeekPublished", (req, res) => {
    const managerId = req.body.id;
    Week.findOneAndUpdate({ name: "NextWeek", ofManager: managerId }, { publishScheduling: "true" })
        .then((response) => {
            res.status(200).json(response);
        })
        .catch((err) => {
